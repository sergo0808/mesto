...
export const imgButtonClose = document.querySelector(
...
const handleLikeClick = (item) => {
  api.likeCardApi(item._id).then((res) => {
...
function addCard(input) {
  api.addCardApi(input).then((input) => {
    createCard(input);
    popupAdd.close();
  });
}
...
function saveDataFormEdit(input) {
  api.updateUserInfom(input).then((input) => {
